...
MODEL_PATH = BASE_DIR / "food_classifier_resnet50.pth"
...
@st.dialog("Supported Food Classes Overview", width="large")
...
st.markdown(
    ...
    <div style="display: inline-flex; align-items: center; gap: 8px; padding: 4px 12px; border-radius: 6px; background: rgba(0, 229, 255, 0.08); border: 1px solid rgba(0, 229, 255, 0.2); color: #00E5FF; font-family: 'JetBrains Mono', monospace; font-size: 0.75rem; margin-bottom: 0.75rem;">
        <span>COMPUTER VISION / RESNET-50</span>
    </div>
    """,
    unsafe_allow_html=True
)
...
with btn_col:
    if st.button("View All Food Types", use_container_width=True):
        show_classes_modal(class_names)

st.markdown(
    """
    <p style="color: #9CA3AF; font-size: 0.95rem; margin-top: 0.5rem; margin-bottom: 2rem;">
        Upload a food image to evaluate fine-tuned ResNet-50 feature representations and probability distributions in real time.<br>
...
if uploaded_file is not None and model is not None:
    ...
    classify_click = st.button("Classify Food Image", use_container_width=True)

    if classify_click:
        with st.spinner('Running tensor transformations & ResNet-50 inference...'):
        ...
        with res_col1:
            ...
            st.markdown(
                ...
                <div class="result-badge">
                    <span style="text-transform: capitalize;">{predicted_class}</span>
